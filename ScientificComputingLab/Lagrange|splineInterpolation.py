...
def constant_eval(x_values, y_values):
    ...
    for i in range(1,n-1):
        ...
        equation = Ms[i-1]+4*Ms[i]+Ms[i+1] - (6/h**2)*(y_values[i-1]-2*y_values[i]+y_values[i+1])
...
def cubicbSpline(xi,x, xii, mi, mii, yi, yii):
    y = (1/6)*mi*(xii-x)**3 + (1/6)*mii*(x-xi)**3+(xii-x)*(yi-(1/6)*mi)+(x-xi)*(yii-(1/6)*mii)
    return y
...
print(cubicbSpline(sides[0], x, sides[1], constants[sides[2]], constants[sides[3]], y_values[sides[2]], y_values[sides[3]]))
